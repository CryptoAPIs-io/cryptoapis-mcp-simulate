...
const CRYPTOAPIS_SERVER_INFO = {
    name: "cryptoapis-simulate",
    version: "0.1.0",
    title: "CryptoAPIs Simulate",
...
export async function startSimulateServer(opts: StartOptions) {
    ...
    if (opts.transport === "stdio") {
        ...
        logger.logInfo("cryptoapis-simulate MCP running (stdio)");
        return;
    }
    ...
    app.listen(port, host, () => {
        logger.logInfo(`cryptoapis-simulate MCP running (http) at http://${host}:${port}${path}`);
        if (cfg.apiKey) {
